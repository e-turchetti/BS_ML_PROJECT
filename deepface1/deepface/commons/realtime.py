...
import pandas as pd
import cv2
from deepface1.deepface import DeepFace
from deepface1.deepface.commons import functions
...
def analysis(
    ...
    while True:
        ...
        if face_detected == True and face_included_frames == frame_threshold and freeze == False:
            ...
            if detected_faces_final[0]==(0,0,640,480) : 
                detected_bool=False
            else: 
                detected_bool=True
            print(detected_bool)
        ...
        if freeze == True:
            ...
            if (toc - tic) < time_threshold:
                ...
                if freezed_frame == 0:
                    ...
                    for detected_face in detected_faces_final:
                        ...
                        cv2.rectangle(
                            freeze_img, (x, y), (x + w, y + h), (255,0,0), 1
                        )  # draw rectangle to main image
                        ...
                        if len(dfs) > 0: 
                            ...
                            df = dfs[0]
                            if df.shape[0] > 0: #è vero quando riconosce qualcuno 
                                candidate = df.iloc[0]
                            ...
                            if detected_bool==False:
                                counter1=0
                                counter2=0
                            else: 
                                if df.shape[0]>0 and counter1==0:
                                    label_old=label
                                    counter1+=1
                                    counter2=0
                                elif df.shape[0]>0 and counter1!=0 and label==label_old:
                                    counter1+=1
                                    counter2=0
                                elif df.shape[0]==0:
                                    counter1=0
            ...
            cv2.imshow("img", img)
        if counter2==recognition_times and counter1!=recognition_times:
            print('Volto trovato ma sconosciuto')
            break 
        if counter1==recognition_times:
            print('Ciao '+ label)
            break
